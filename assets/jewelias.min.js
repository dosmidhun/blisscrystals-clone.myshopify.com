!function(e) {
    ...
    var f = {
        ...
        checkNeedToConvertCurrency: function() {
            return window.show_multiple_currencies && Currency.currentCurrency != window.shopCurrency
        },
            ...
            e(".add-to-cart-btn").length > 0 && e(document).on("click", ".add-to-cart-btn", function(t) {
                ...
                if ("disabled" != e(this).attr("disabled")) {
                    var o = e(this).parents(".product-item")
                      , a = (e(o).attr("id"),
                    e(o).find(".product-grid-image").data("handle"),
                    e(o).find("form"));
                    if (window.ajax_cart) {
                        ...
                        var r = e(o).find(".product-title").html()
                          ...
                          , d = e(o).find(".product-grid-image img").attr("src");
                        f.doAjaxAddToCart(i, r, l, d)
                    } else
                        e(this).closest("form").submit()
                }
                return !1
        ...
        doAjaxAddToCart: function(t, i, o, a) {
            e.ajax({
                ...
                success: function(t) {
                    f.hideLoading(),
                    e(".ajax-success-modal").find(".ajax-product-title").html(f.translateText(i)),
        ...
        doAjaxInfiniteScrollingGetContentSection(t, i, o, a, n) {
            e.ajax({
                ...
                success: function(t) {
                    if (n.find(".products-grid").append(t),
                    e(t).length == i ? e(a.currentTarget).attr("data-page", o + 1) : (e(a.currentTarget).attr("disabled", "disabled"),
                    e(a.currentTarget).addClass("disabled"),
                    e(a.currentTarget).text(window.inventory_text.no_more_product)),
                    f.initWishLists(),
                    f.initWishListIcons(),
                    f.checkNeedToConvertCurrency() && Currency.convertAll(window.shop_currency, e("#currencies .active").attr("data-currency"), "span.money", "money_format"),
                    e(".shopify-product-reviews-badge").length && e(".spr-badge").length)
                        return window.SPR.registerCallbacks(),
                        window.SPR.initRatingHandler(),
                        window.SPR.initDomEls(),
                        window.SPR.loadProducts(),
                ...
                complete: function() {
                    f.hideLoading(),
                    e(a.currentTarget).removeClass("ajax-loading"),
                    e(".infinite-scrolling1").remove()
                }
            })
        },
        initQuickView: function(o) {
            var a = function(t, i) {
                ...
                if (t) {
                    ...
                    e(".quick-view #product_regular_price").val(t.price),
                    ...
                    t.compare_at_price > t.price) {
                        ...
                        n.addClass("on-sale");
                        var d = Math.round(100 * (1 - t.price / t.compare_at_price));
                        l.html("-" + d + "%"),
                        l.show(),
                        r.html("-" + Math.floor(100 * (t.compare_at_price - t.price) / t.compare_at_price) + "%").show()
                    } else
                        s.hide(),
                    ...
                    if (window.use_color_swatch)
                        for (var c = e("#" + i.domIdPrefix).closest("form"), p = 0, u = t.options.length; p < u; p++) {
                            var h = c.find('.swatch[data-option-index="' + p + '"] :radio[value="' + t.options[p] + '"]');
                            h.size() && (h.get(0).checked = !0)
                        }
                    if (window.display_quickview_availability) {
                        var g = o.find(".product-inventory span");
                        t.available ? null != t.inventory_management ? g.text(window.inventory_text.in_stock) : g.text(window.inventory_text.many_in_stock) : g.text(window.inventory_text.out_of_stock)
                    }
                    if (window.display_quickview_sku) {
                        var w = o.find(".sku-product span");
                        t ? w.text(t.sku) : w.empty()
                    }
                    ...
                    t && t.featured_image) {
                        var m = e(".quick-view .quickview-featured-image img")
                          , v = t.featured_image
                          , b = m[0];
                        Shopify.Image.switchImage(v, b, function(t, i, o) {
                            t = t.replace(/\?(.*)/, ""),
            ...
            t.off("click.initQuickView", ".quickview-button a").on("click.initQuickView", ".quickview-button a", function(t) {
                ...
                e.ajax({
                    ...
                    success: function(t) {
                        ...
                        Shopify.getProduct(i, function(t) {
        ...
        updatePricingQuickview: function() {
            ...
            e(".quick-view .total-price span").html(o),
            f.checkNeedToConvertCurrency() && Currency.convertAll(window.shop_currency, e("#currencies .active").attr("data-currency"), "span.money", "money_format")
        },
        intslick: function() {
        ...
        createWishListTplItem: function(t) {
            ...
            jQuery.getJSON(window.router + "/products/" + t + ".js", function(t) {
                ...
                o += '<div class="inner product-item" data-product-id="product-' + t.handle + '">',
                o += '<div class="row2">',
                o += '<div class="product-image col-md-2">',
                o += '<a href="' + t.url + '" class="product-grid-image" data-collections-related="/collections/all?view=related">',
                o += '<img src="' + t.featured_image + '" alt="' + t.featured_image.alt + '">',
                o += "</a></div>",
                o += '<div class="product-bottom col-md-4">',
                o += '<a class="product-title" href="' + t.url + '" title="' + t.title + '">' + t.title + "</a>",
                o += '<div class="product-vendor">',
                o += '<a href="' + window.router + "/collections/vendors?q=" + t.vendor + '" title="' + t.vendor + '">' + t.vendor + "</a></div></div>",
                o += '<div class="price-box col-md-2">' + a + "</div>",
                o += '<div class="col-md-3 wishlist-action">',
                o += '<form action="/cart/add" method="post" class="variants" data-id="product-actions-' + t.id + '" enctype="multipart/form-data">',
                t.available ? (1 == t.variants.length && (o += '<button data-btn-addToCart class="btn add-to-cart-btn" type="submit">' + window.inventory_text.add_to_cart + '</button><input type="hidden" name="id" value="' + t.variants[0].id + '" />'),
                1 < t.variants.length && (o += '<a class="btn" title="' + t.title + '" href="' + t.url + '">' + window.inventory_text.select_options + "</a>")) : o += '<button class="btn add-to-cart-btn" type="submit" disabled="disabled">' + window.inventory_text.unavailable + "</button>",
                o += "</form></div>",
                o += '<div class="btn-remove col-md-1"><a class="whislist-added" href="#" data-product-handle="' + t.handle + '" data-icon-wishlist data-id="' + t.id + '"><svg class="closemnu" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 357 357" xml:space="preserve"><g><g><polygon points="357,35.7 321.3,0 178.5,142.8 35.7,0 0,35.7 142.8,178.5 0,321.3 35.7,357 178.5,214.2 321.3,357 357,321.3 214.2,178.5"></polygon></g></g></svg></a></div>',
                o += "</div></div></div>",
                i.append(o);
